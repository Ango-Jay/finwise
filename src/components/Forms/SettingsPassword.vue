...
const rules = {
  ...
  newPassword: {
    ...
    mustNotEqual:helpers.withMessage("Old and New password cannot be the same", mustNotEqual )
  },
  confirmPassword: {
    ...
    setTimeout(async()=>{
        try {
            const checkifOldPasswordIsValid = ()=>{
                const currentUser = users.value.find(item => item.uniqueId === user.value.id)
  ...
  <div class="w-full px-10 mt-[3rem]">
    <div class="w-full mb-8">
        <div class="flex flex-col w-full">
          ...
          <button
            type="button"
            @click="showPassword.currentPassword = !showPassword.currentPassword"
